...
#
#    fake_wls.edit.changeManager.startEdit.assert_called_once()
#    fake_wls.edit.changeManager.activate.assert_called_once()
#    fake_wls.edit.changeManager.cancelEdit.assert_not_called()
#
# def test_wls_with_edit_session_no_activate():
#    fake_wls = MagicMock(spec=wls_rest_python.WLS)
#    fake_wls.edit = MagicMock()
#    with wls_rest_python.WLS.edit_session(fake_wls, activate=False):
#        fake_wls.edit.tjohei()
#
#    fake_wls.edit.changeManager.startEdit.assert_called_once()
...
#
#    fake_wls.edit.changeManager.startEdit.assert_called_once()
#    fake_wls.edit.changeManager.cancelEdit.assert_called_once()
#    fake_wls.edit.changeManager.activate.assert_not_called()
#
...
def test_wls_get():
    ...
    fake_wls.session.get.assert_called_once_with(
        "https://url", timeout=372, weird_requests_option="hei"
    )
...
def test_wls_post():
    ...
    fake_wls.session.post.assert_called_once_with(
        "https://url", headers=None, timeout=372, weird_requests_option="hei"
    )
...
def test_wls_delete():
    ...
    fake_wls.session.delete.assert_called_once_with(
        "https://url", headers=None, timeout=372, weird_requests_option="hei"
    )
...
def test_wls_handle_response_job():
    job = {
        "links": [{"rel": "job", "href": "https://joblink"}],
...
def test_wls_handle_response_unknown():
    what = {"what": True, "is": False, "this": "true story"}
    fake_wls = MagicMock(spec=wls_rest_python.WLS)
    response = MagicMock()
    response.ok = True
    response.json = MagicMock(return_value=what)
...
def test_wls_handle_unknown_error():
    ...
    with pytest.raises(wls_rest_python.WLSException, match="Weird error"):
        wls_rest_python.WLS._handle_error(response)
